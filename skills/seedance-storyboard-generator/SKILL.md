...
description: 面向任意AI Agent的Seedance 2.0/1.5视频脚本与分镜生成技能。当用户要求：(1) 将文章/故事转换为视频脚本，(2) 生成Seedance分镜提示词，(3) 规划多集AI视频系列，(4) 为Nana Banana Pro或其他图像模型创建角色/场景/道具生成提示词时使用。输入可以是完整小说、文章或简短故事大纲。输出包括标准脚本格式完整剧本（△镜头描述+对白+OS/VO+闪回+字幕）、剧集分解、资产生成提示词和Seedance 2.0/1.5格式分镜脚本。
---

# Seedance 分镜生成器

专业的AI脚本和分镜生成系统，用于在Seedance 2.0/1.5平台上创建专业的AI视频系列。

## Agent-Agnostic Contract

- 该技能不依赖特定模型或产品，可被任意支持 Markdown 指令的 Agent 使用。
- 调用时向 Agent 提供：故事素材（全文或大纲）+ 目标风格 + 时长/集数 + 画幅 + 平台偏好 + 参考素材。
- 默认输出交付：
  1. `[Title]_剧本.md`
  2. `[Title]_素材清单.md`
  3. `[Title]_E[XX]_分镜.md`（按集生成）
- 若用户信息不完整，先提澄清问题，再进入工作流。
...
### 5. 生成 Seedance 分镜脚本（2.0 / 1.5）

**先确认版本：**
- **Seedance 2.0**：支持视频延长，可用 `将@视频1延长15s` 做集间串联。
- **Seedance 1.5**：不支持视频延长，只能用首帧/尾帧衔接；必须显式写转场承接运镜。
...
| 图片2 | S01 | 草料场场景参考 |
| 图片3 | E01_END | 上一段尾帧（1.5模式下作为本段首帧约束） |
```

**b1. Seedance 2.0 Prompt（时间轴格式，使用中文）**
```
...
**b2. Seedance 1.5 Prompt（首帧/尾帧模式，使用中文）**
```
[风格描述]，15秒，9:16竖屏，[整体氛围]
【版本】Seedance 1.5（首尾帧模式）
【首帧约束】@图片1 作为本段首帧（通常来自上一段尾帧）
【尾帧目标】@图片2 作为本段尾帧目标（用于下一段承接）

0-3秒：[承接上一段的转场运镜] - [镜头运动]，[首帧到动作起点的连续变化]
...
12-15秒：[结尾/交接] - [镜头运动]，[最终画面]，[为下一段准备的转场锚点]

【声音】[配乐风格] + [音效] + [对白/旁白]

【参考】@图片1 [首帧用途说明]，@图片2 [尾帧用途说明]，@图片3 [角色/场景补充]
```

**c. Ending Frame Description（2.0/1.5通用）**
- 记录最后一帧内容，确保下一集的连续性
- 包含：主体、背景、光线、构图、氛围

**Camera movement keywords:** 推镜头/拉镜头/摇镜头/移镜头/跟镜头/环绕镜头/升降镜头/希区柯克变焦/一镜到底/手持晃动

**d. Seedance 1.5 集间衔接规则（必须执行）**

1. **先判断“下一段首帧”是否能直接承接“上一段尾帧”**
   - 能承接：下一段提示词必须显式写：
     - `上一段视频尾帧，也就是本次视频的首帧：[...]。`
     - `请承接上一段的[缓慢拉远/右摇/跟拍等]转场运镜，继续[...]。`
   - 同时保持运镜参数连续：方向、速度、景别、透视、光线、主体朝向。

2. **若不能直接承接，必须做“转场接力”**
   - 上一段最后1-2秒写清转场运镜（例如：拉远并轻微虚焦、向右横移过渡、遮挡切换）。
   - 下一段开头0-3秒明确“承接该转场运镜”，再落到新场景主动作。
   - 不能只写“切到下一幕”，必须有可执行的镜头语言。

3. **推荐写法模板**
   - 直接承接模板：
     - `上一段视频尾帧，也就是本次视频的首帧在上一段视频镜头缓慢拉远。请承接这个转场运镜，继续从中景拉远过渡到全景，展现[...]。`
   - 转场接力模板：
     - 上一段结尾：`12-15秒：镜头向右横移并渐进虚焦，掠过前景遮挡，停在门框轮廓。`
     - 下一段开场：`0-3秒：承接上一段向右横移+虚焦转清晰，从门框轮廓进入室内中景，继续[...]。`

**For episode chaining (Ep 2+):**
- Seedance 2.0：开始提示词使用 `将@视频1延长15s` 并上传上一集作为视频参考。
- Seedance 1.5：不使用视频延长，使用“上一段尾帧 -> 本段首帧”的首尾帧承接规则。
...
3. **[Title]_E[XX]_分镜.md** - 单集分镜脚本，包含时间轴格式（2.0或1.5）
4. **[Title]_E[XX]_分镜_Seedance1.5.md** - 首尾帧承接强化版（仅当目标为1.5时输出）
...
- ✅ 若为2.0：已正确使用 `将@视频1延长15s` 与上集视频参考
- ✅ 若为1.5：每集都提供“首帧来源 + 尾帧目标 + 转场承接运镜”
- ✅ 若为1.5且首尾帧不可直承：上下两集均写明成对转场接力运镜

## 参考材料

有关详细的Seedance 2.0/1.5提示词模式、模板和最佳实践，请参阅 [references/seedance-manual.md](references/seedance-manual.md)。
...
8. **忽略1.5衔接约束**：1.5不能靠视频延长，必须写清“上一段尾帧 -> 本段首帧”的承接运镜或转场接力。
