...
description: 面向任意AI Agent的Seedance 2.0视频脚本与分镜生成技能。当用户要求：(1) 将文章/故事转换为视频脚本，(2) 生成Seedance 2.0分镜提示词，(3) 规划多集AI视频系列，(4) 为Nana Banana Pro或其他图像模型创建角色/场景/道具生成提示词时使用。输入可以是完整小说、文章或简短故事大纲。输出包括标准脚本格式完整剧本（△镜头描述+对白+OS/VO+闪回+字幕）、剧集分解、资产生成提示词和Seedance 2.0格式分镜脚本。
---
